...
use std::{collections::HashMap, fmt::Debug, hash::Hash, vec, borrow::Borrow};

use rusted_gumtree_core::tree::tree::{
    LabelStore as LabelStoreTrait, NodeStore as NodeStoreTrait, Type, Typed, VersionedNodeStore, Stored, VersionedNodeStoreMut,
};
...
use crate::{
    ...
    store::{
        vec_map_store::{AsNodeEntityRef, AsNodeEntityRefSelf, SymbolU32, VecMapStore},
        TypeStore,
    },
    tree_gen::{
...
pub struct HashedNode(
    HashedCompressedNode<SyntaxNodeHashs<HashSize>, SymbolU32<HashedNode>, LabelIdentifier>,
);

impl AsRef<HashedNode> for HashedNode {
    fn as_ref(&self) -> &HashedNode {
        self
    }
}

type NodeIdentifier = SymbolU32<HashedNode>;
...
impl AsNodeEntityRefSelf for HashedNode {
...
impl<'a> NodeStoreTrait<'a, NodeIdentifier, &'a HashedNode> for NodeStore {
    fn resolve(&'a self, id: &NodeIdentifier) -> &'a HashedNode {
        self.internal.resolve(id)
    }
}

impl<'a> NodeStoreMutTrait<'a, HashedNode, &'a HashedNode> for NodeStore {
}
...
impl<'a> NodeStore {
    fn get_or_insert(&mut self, node: HashedNode) -> NodeIdentifier {
        self.count += 1;
        if node.get_type() == Type::Error && self.internal.get::<&HashedNode>(&node).is_none() {
            self.errors += 1;
            println!("{:?}", &node);
        }
...
impl<'a> VersionedNodeStore<'a, NodeIdentifier, &'a HashedNode> for NodeStore {
    fn resolve_root(&self, version: (u8, u8, u8), node: <HashedNode as Stored>::TreeId) {
        todo!()
    }
}

impl<'a> VersionedNodeStoreMut<'a, HashedNode, &'a HashedNode> for NodeStore {
...
pub fn print_tree_structure(node_store: &NodeStore, id: &NodeIdentifier) {
    nodes::print_tree_structure(
        |id| -> _ { &node_store.resolve(id).0.node },
        id,
    )
}
