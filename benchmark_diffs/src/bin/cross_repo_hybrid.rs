...
use hyperast_benchmark_diffs::{
    cross_repo_hybrid::{CommitCompareParameters, windowed_commits_compare},
    setup_env_logger,
};
...
fn main() {
    ...
    //     }
    // });
    let out_perfs = args.get(1).and_then(|x| {
        if x.is_empty() {
            None
        } else {
            Some(PathBuf::from_str(x).unwrap())
    ...
    let limit = args.get(2).and_then(|x| x.parse::<usize>().ok()).unwrap_or(30);
    dbg!(limit);
    ...
    windowed_commits_compare(2, preprocessed, params, limit, out_perfs);
}
