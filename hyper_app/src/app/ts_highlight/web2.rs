...
// use web_tree_sitter_sg::QueryCapture;
// use web_tree_sitter_sg::{SyntaxNode,};
...
impl From<Point> for tree_sitter::Point {
    fn from(value: Point) -> Self {
        tree_sitter::Point::new(value.row as u32, value.column as u32)
    }
}
...
extern "C" {
    ...
    unsafe fn exec(
        this: &QueryCursor,
        query: &web_tree_sitter_sg::Query,
        node: &web_tree_sitter_sg::SyntaxNode,
    ) -> u32;//ts_query_cursor_exec

    #[wasm_bindgen(method, js_name = nextCapture)]
    unsafe fn next_capture(
        this: &QueryCursor,
        aaa: *mut (),
    ...
    // // -> QueryCapture[]
    // #[wasm_bindgen(method, js_name = captures)]
    // fn _captures(
    //     this: &QueryCursor,
    //     query: &web_tree_sitter_sg::Query,
    //     node: &web_tree_sitter_sg::SyntaxNode,
...
impl QueryCursor {
    // -> QueryMatch[]
    pub fn matches<'a, 'tree>(
        &self,
        query: &web_tree_sitter_sg::Query,
        node: &web_tree_sitter_sg::SyntaxNode,
        text: &[u8],
    ) -> QueryMatches<'a, 'tree, &'a [u8]> {
        let this = unsafe { std::mem::transmute(self)};
        eframe::web_sys::console::log_1( this);
        todo!()
        // QueryMatches {
        //     tmp: self._matches(query, node, text),
        //     phantom: Default::default(),
        // }
    }

    // -> QueryCapture[]
    pub fn captures<'a, 'tree, T: 'a>(
        &'a mut self,
        query: &'a web_tree_sitter_sg::Query,
        node: Node<'tree>,
        text_provider: T,
        ...
        eframe::web_sys::console::log_1( unsafe { std::mem::transmute(&node)});
        let node = unsafe { std::mem::transmute(&node)};
        // safety: self is mutably borrowed
        unsafe { self.exec(query, node) };
        ...
        QueryCaptures {
            ...
            buffer2: Default::default(),
            _tree: PhantomData,
            // tmp: self._captures(query, node, text),
            // text:text_provider,
            // phantom: PhantomData,
        }
    }
...
pub struct QueryMatches<'a, 'tree, T> {
    tmp: Box<[JsValue]>,
    phantom: PhantomData<&'a &'tree T>,
}

impl<'a, 'tree, T> QueryMatches<'a, 'tree, T> {
    fn pattern_index(&self) -> usize {
        todo!()
    }
}

impl<'a, 'tree, T> Iterator for QueryMatches<'a, 'tree, T> {
...
pub struct QueryCaptures<'a, 'tree, T> {
    ptr: QueryCursor,//*mut ffi::TSQueryCursor,
    query: &'a web_tree_sitter_sg::Query,
    text_provider: T,
    buffer1: Vec<u8>,
    buffer2: Vec<u8>,
...
impl<'a, 'tree, T> Iterator for QueryCaptures<'a, 'tree, T> {
    type Item = (QueryMatch<'a,'a>, usize);//(QueryMatch<'a, 'tree>, usize)

    fn next(&mut self) -> Option<Self::Item> {
        unsafe {
            loop {
                let mut capture_index = 0u32;
                // let mut m = std::mem::MaybeUninit::<ffi::TSQueryMatch>::uninit();
                // if ffi::ts_query_cursor_next_capture(
                //     self.ptr,
                //     m.as_mut_ptr(),
                //     &mut capture_index as *mut u32,
                // ) {
                let mut m = std::mem::MaybeUninit::<_>::uninit();
                if self.ptr.next_capture(
                    m.as_mut_ptr(),
                    &mut capture_index as *mut u32,
                ) {
...
mod wb {
    ...
    extern "C" {
        ...
        #[wasm_bindgen(method, getter)]
        pub fn index(this: &QueryCapture) -> u32;
...
struct QueryMatch<'a, 'tree> {
    inner: web_tree_sitter_sg::QueryMatch,
    phantom: PhantomData<&'a &'tree ()>,
}

impl<'a, 'tree> Deref for QueryMatch<'a, 'tree> {
    type Target = web_tree_sitter_sg::QueryMatch;
...
impl<'a, 'tree> QueryMatch<'a, 'tree> {
    fn pattern_index(&self) -> usize {
        self.inner.pattern() as usize
    }
    fn captures(&self) -> &[QueryCapture] {
        self.inner.captures();
        todo!()
    }
    fn remove(&self) {
...
pub struct QueryMatch<'cursor, 'tree> {
    ...
    id: u32,
    cursor: QueryCursor, //*mut ffi::TSQueryCursor,
}
...
impl Deref for Query {
    type Target = web_tree_sitter_sg::Query;
...
impl Query {
    pub fn new(language: &Language, source: &str) -> Result<Self, QueryError> {
        tree_sitter::Query::new(language, source).map(Query)
    }
    fn pattern_count(&self) -> usize {
        let sefl: &web_tree_sitter_sg::Query = unsafe { std::mem::transmute(self) };
        let predicates = js_sys::Reflect::get(sefl, &"predicates".into()).unwrap();
        assert!(predicates.is_array());
        js_sys::Array::from(&predicates).length() as usize
    }
    fn start_byte_for_pattern(&self, pattern_index: usize) -> usize {
        let this = unsafe { std::mem::transmute(&self.0)};
        eframe::web_sys::console::log_1( this);
        let fct = js_sys::Reflect::get(this, &"startByteForPattern".into()).unwrap();
        let aaa = js_sys::Function::call1(&JsCast::unchecked_into(fct), this, &pattern_index.into()).unwrap();
        eframe::web_sys::console::log_1( &aaa);
        // let predicates = js_sys::Reflect::get(self, &"predicates".into()).unwrap();
        aaa.as_f64().unwrap() as usize
    }
    ...
    fn capture_names(&self) -> Vec<String> {
        let this = unsafe { std::mem::transmute(&self.0)};
        let names = js_sys::Reflect::get(this, &"captureNames".into()).unwrap();
        let aaa: Vec<String> = serde_wasm_bindgen::from_value(names).unwrap();
        aaa
    }
}
...
impl<'a> HighlightIterLayer<'a> {
    ...
    fn new<F: FnMut(&str) -> Option<&'a HighlightConfiguration> + 'a>(
        ...
        loop {
            let rrr: Vec<tree_sitter::Range> = ranges.iter().map(|x| x.clone().into()).collect();
            if highlighter.parser.set_included_ranges(&rrr).is_ok() {
                highlighter
                    .parser
                ...
                if let Some(combined_injections_query) = &config.combined_injections_query {
                    ...
                    let matches = cursor.matches(
                        combined_injections_query,
                        unsafe { std::mem::transmute(&tree.root_node()) },
                        source,
                    );
                    for mat in matches {
    ...
    fn intersect_ranges(
        ...
        for node in nodes.iter() {
            let mut preceding_range = Range {
                start_byte: 0,
                start_point: Point::new(0, 0),
                end_byte: node.start_byte() as usize,
                end_point: node.start_position().into(),
            };
            let following_range = Range {
                start_byte: node.end_byte() as usize,
                start_point: node.end_position().into(),
                end_byte: usize::MAX,
                end_point: Point::new(usize::MAX, usize::MAX),
            };
            ...
            for excluded_range in node
                ...
                .filter_map(|child| {
                    if includes_children {
                        ...
                        Some(child.range().into())
                    }
                })
                .chain([following_range].iter().cloned())
            {
                let excluded_range: Range = excluded_range.into();
                let mut range = Range {
    ...
    fn sort_key(&mut self) -> Option<(usize, bool, isize)> {
        ...
        let next_start = self
            .captures
            .peek()
            .map(|(m, i)| m.captures[*i].node.start_byte() as usize);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        'main: loop {
            ...
            if let Some((next_match, capture_index)) = layer.captures.peek() {
                let next_capture = next_match.captures[*capture_index].clone();
                let node = &next_capture.node;
                range = (node.start_byte() as usize)..node.end_byte() as usize; //byte_range();
            ...
            let mut capture = match_.captures[capture_index].clone();
                ...
                // local scope at the top of the scope stack.
                    ...
                    for capture in match_.captures {
                        if Some(capture.index) == layer.config.local_def_value_capture_index {
                            ...
                            value_range = (node.start_byte() as usize)..node.end_byte() as usize;
                            //byte_range();
                        }
                    }
...
fn injection_for_match<'a>(
    ...
    for capture in query_match.captures {
        ...
        if index == language_capture_index {
            language_name = from_utf8(
                &source[capture.node.start_byte() as usize..capture.node.end_byte() as usize],
            )
            .ok(); //capture.node().utf8_text(source).ok();
        } else if index == content_capture_index {
            content_node = Some(capture.node.clone());
        }
    }
