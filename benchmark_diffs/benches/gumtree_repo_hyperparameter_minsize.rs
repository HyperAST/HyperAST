...
fn diff_benchmark(c: &mut Criterion) {
    env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("trace"))
        .is_test(true)
        .init();
    let mut group = c.benchmark_group("gumtree_repo_hyperparameter_minsize");
    ...
    let mut buf_perfs = BufWriter::with_capacity(4 * 8 * 1024, File::create("/tmp/repo_minsize.csv").unwrap());
    writeln!(
        buf_perfs,
    ...
    ($($threshold:expr),*) => {
            $(
                {
                    // group.bench_with_input(BenchmarkId::new("repo_hybrid_hyperparameter_min_size", $threshold), &$threshold,|b, i| {
                    //     b.iter_custom(|iters| {
