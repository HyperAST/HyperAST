...
fn main() {
    ...
    let codes = It::new(Path::new(&codes).to_owned()).map(|x| {
        let text = std::fs::read_to_string(&x).expect(&format!("{:?} is not a java file or a dir containing java files: ", x));
        (x, text)
    });
    ...
    let queries: Vec<_> = It::new(Path::new(&queries).to_owned())
        .map(|x| {
            let text = std::fs::read_to_string(&x)
                .expect(&format!("{:?} in not a file of treesitter queries of a dir containing such files", x));
            (x, text)
        })
        .collect();
    compare_all(codes, &queries);
