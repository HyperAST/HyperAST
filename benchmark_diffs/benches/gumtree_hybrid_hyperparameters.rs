...
fn diff_benchmark(c: &mut Criterion) {
    ...
    let (_, buggy, fixed) = test_inputs.get(1).unwrap();
            ...
            $(
                {
                    ...
                    group.bench_with_input(BenchmarkId::new("hybrid_hyperparameter", SIZE_THRESHOLD), &SIZE_THRESHOLD,|b, i| {
                        b.iter(|| {
                            run_diff::<SIZE_THRESHOLD>(buggy, fixed);
                        })
                    });
                }
            )*
    ...
    run_diff_for_thresholds!(0, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500);
    group.finish();
}
...
fn run_diff<const SIZE_THRESHOLD: usize>(src: &str, dst: &str) {
    ...
    let diff_result= algorithms::gumtree_hybrid::diff_hybrid::<_, SIZE_THRESHOLD>(
            &stores,
