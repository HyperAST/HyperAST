...
const A1_CONTENT: &str = include_str!("../tests/A.java");
const A2_CONTENT: &str = include_str!("../tests/A2.java");

fn diff_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("sum_comparison");
    
    group.bench_function("gumtree lazy", |b| {
        // Setup (none)
        b.iter(|| {
            // Actual measurement
    ...
    group.bench_function("gumtree", |b| {
        // Setup (none)
        b.iter(|| {
            // Actual measurement
