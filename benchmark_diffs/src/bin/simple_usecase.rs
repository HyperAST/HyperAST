...
fn inc(mut preprocessed: PreProcessedRepository, before: &str, after: &str, out: Option<PathBuf>) {
    ...
    for i in 0..100 {
        ...
        let mu = memusage_linux();
        let lazy =
            hyper_diff::algorithms::gumtree_lazy::diff(&hyperast, &src_tr, &dst_tr, 1000, 0.5f64);
...
fn whole(
    ...
    for c in (0..c_len - 1).map(|c| &processing_ordered_commits[c..(c + window_size).min(c_len)]) {
        ...
        for oid_dst in &c[1..] {
            ...
            let mu = memusage_linux();
            let lazy = hyper_diff::algorithms::gumtree_lazy::diff(
                &hyperast, &src_tr, &dst_tr, 1000, 0.5f64,
            );
            let summarized_lazy = &lazy.summarize();
            use hyper_diff::algorithms::ComputeTime;
            let total_lazy_t: std::time::Duration = summarized_lazy.time();
