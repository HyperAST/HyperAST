...
use std::{fmt::Debug, io::stdout, vec, collections::HashMap};
...
pub struct CppTreeGen<'store, 'cache> {
    pub line_break: Vec<u8>,
    pub stores: &'store mut SimpleStores,
...
impl<'store, 'cache> ZippedTreeGen for CppTreeGen<'store, 'cache> {
    ...
    fn init_val(&mut self, text: &[u8], node: &Self::Node<'_>) -> Self::Acc {
        ...
        let kind = type_store.get_cpp(node.kind());
    ...
    fn pre_skippable(
        ...
        if kind == Type::cpp_StringLiteral || kind == Type::cpp_NumberLiteral || kind == Type::cpp_CharLiteral {
            *skip = true;
        }
...
impl<'store, 'cache> CppTreeGen<'store, 'cache> {
    fn make_spacing(
        &mut self,
...
impl<'stores, 'cache> TreeGen for CppTreeGen<'stores, 'cache> {
    type Acc = Acc;
