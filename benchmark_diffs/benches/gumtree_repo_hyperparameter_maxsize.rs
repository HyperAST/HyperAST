...
fn diff_benchmark(c: &mut Criterion) {
    ...
    let mut buf_perfs = BufWriter::with_capacity(4 * 8 * 1024, File::create("/tmp/repo_maxsize.csv").unwrap());
    writeln!(
        buf_perfs,
    ...
    for max_size in values {
        group.bench_with_input(BenchmarkId::new("repo_hybrid_hyperparameter", max_size), &max_size,|b, i| {
            b.iter_custom(|iters| {
                ...
                for _ in 0..iters {
                    for ((stores, src_tr, dst_tr), before, after) in dataset_trees.iter() {
                        ...
                        let summary = run_diff_trees(stores, src_tr, dst_tr, "hybrid", max_size);
                        epoch::advance().unwrap();
