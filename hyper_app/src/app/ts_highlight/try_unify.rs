...
use tree_sitter_native::{LossyUtf8, QueryCaptures, QueryCursor, QueryMatch};

#[cfg(target_arch = "wasm32")]
use web_tree_sitter_sg::{LossyUtf8, QueryCaptures, QueryCursor, QueryMatch};
...
impl HighlightConfiguration {
    ...
    pub fn new(
        ...
        if cfg!(wasm) {
            ...
            for i in 0..(query.inner.pattern_count()) {
                let pattern_offset = query.inner.start_byte_for_pattern(i);
        ...
        if cfg!(wasm) {
            ...
            for pattern_index in 0..locals_pattern_index {
                let settings = query.inner.property_settings(pattern_index);
                if settings.iter().any(|s| &*s.key == "injection.combined") {
                    has_combined_queries = true;
                    query.inner.disable_pattern(pattern_index);
                } else {
                    combined_injections_query
                        .inner
                        .disable_pattern(pattern_index);
                }
            }
        };
        ...
        let non_local_variable_patterns = if cfg!(wasm) {
            todo!()
        } else {
            (0..query.inner.pattern_count())
                .map(|i| {
                    query
                        .inner
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            capture_names = query.inner.capture_names();
        }
        #[cfg(target_arch = "wasm32")]
        {
            capture_names = web_tree_sitter_sg::Query::capture_names(&query.inner);
        }
    ...
    pub fn names(&self) -> &[String] {
        if cfg!(wasm) {
            ...
            self.query.inner.capture_names()
        }
...
impl<'a> HighlightIterLayer<'a> {
    ...
    fn new<F: FnMut(&str) -> Option<&'a HighlightConfiguration> + 'a>(
        ...
        loop {
            if highlighter.parser.set_included_ranges(&ranges).is_ok() {
                ...
                if let Some(combined_injections_query) = &config.combined_injections_query {
                    let mut injections_by_pattern_index = vec![
                        ...
                        if cfg!(wasm) {
                            ...
                            combined_injections_query.inner.pattern_count()
                        }
                    ];
                    let matches = if cfg!(wasm) {
                        todo!()
                    } else {
                        cursor.matches(
                            &combined_injections_query.inner,
                            tree.root_node().inner,
                            source,
                        )
                    };
                    for mat in matches {
                ...
                let captures = if cfg!(wasm) {
                    todo!()
                } else {
                    cursor_ref
                        .captures(&config.query.inner, tree_ref.root_node().inner, source)
                        .peekable()
                };
    ...
    fn intersect_ranges(
        ...
        for node in nodes.iter() {
            let mut preceding_range: Range = R {
                start_byte: 0,
                start_point: Point::new(0, 0),
...
fn injection_for_match<'a>(
    ...
    let property_settings = if cfg!(wasm) {
        ...
        query.inner.property_settings(query_match.pattern_index)
    };
    for prop in property_settings {
        match prop.key.as_ref() {
