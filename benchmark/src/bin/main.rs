...
pub fn print_matched_refeferences_from_canonical_type(
...
fn bench_1_aux(
    ...
    use java_tree_gen::{JavaTreeGen, NodeIdentifier};
    let mut full_nodes: BTreeMap<git2::Oid, _> = BTreeMap::default();
    let mut java_tree_gen = JavaTreeGen {
        line_break: "\n".as_bytes().to_vec(),
        stores: &mut SimpleStores::default(),
        md_cache:&mut Default::default(),
    };
    for oid in rw {
        // if i >= 1000 {
        //     break;
